  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-slate-500">
        Full module history. Past modules are greyed to show what students have already studied.
      </p>
              ...
              <div key={m.id} className="rounded-xl border border-gold/40 bg-white p-5 shadow-sm">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="mt-1 text-xs text-slate-400">Released to students {formatDate(m.sent_at!)}</p>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="rounded-full bg-slate-200 px-2 py-0.5 text-xs font-semibold text-slate-500">Completed</span>
                    </div>
                    {m.description && <p className="mt-0.5 text-xs text-slate-400">{m.description}</p>}
                    <p className="mt-1 text-xs text-slate-400">Taught {formatDate(m.sent_at!)}</p>
                  </div>
                  <a href={m.file_url} target="_blank" rel="noopener noreferrer"
      ...
      {locked.length > 0 && (
        <div className="mt-8">
          <h2 className="mb-3 text-xs font-semibold uppercase tracking-widest text-slate-400">
            Not Yet Released ({locked.length})
          </h2>
          <div className="space-y-2">
            {locked.map((m) => (
              <div key={m.id} className="flex items-start gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4">
                <div className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-amber-100 text-amber-500">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                ...
                <div className="flex-1">
                  <p className="font-medium text-slate-600">{m.title}</p>
                  {m.description && <p className="mt-0.5 text-xs text-slate-400">{m.description}</p>}
                  <p className="mt-1 text-xs text-amber-600 font-medium">Pending release — send from Admin → Modules</p>
                </div>
                <span className="shrink-0 rounded-full bg-amber-100 px-2 py-0.5 text-xs font-semibold text-amber-700">Pending</span>
              </div>
            ))}
          </div>
        </div>
