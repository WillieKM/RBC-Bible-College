...
export default async function ProfessorZoomAttendanceDetailPage({
  params,
  searchParams,
}: {
  ...
  return (
    <div className="max-w-2xl">
      <Link href="/professor/zoom-attendance" className="text-sm text-gold-dark hover:underline">← All Sessions</Link>
      <h1 className="mt-2 text-2xl font-bold text-slate-900">Register — {session.title}</h1>
      ...
      <div className="mt-6">
        <ZoomRollCall
          ...
          pastDates={pastDates}
          returnPath={`/professor/zoom-attendance/${id}`}
        />
      </div>
    </div>
  );
