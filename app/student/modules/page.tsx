  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-slate-500">
        All course modules are listed here. Locked modules will become available once released by the college.
      </p>
      ...
      {locked.length > 0 && (
        <div className={unlocked.length > 0 ? "mt-8" : "mt-6"}>
          ...
          <div className="space-y-3">
            {locked.map((m) => (
              <div key={m.id} className="flex items-start gap-3 rounded-xl border border-slate-200 bg-slate-50 p-5 opacity-70">
                ...
                <div className="flex-1">
                  <p className="font-semibold text-slate-500">{m.title}</p>
                  {m.description && (
                ...
                <span className="shrink-0 rounded-full bg-slate-200 px-2.5 py-0.5 text-xs font-semibold text-slate-500">
                  Locked
                </span>
              </div>
            ))}
